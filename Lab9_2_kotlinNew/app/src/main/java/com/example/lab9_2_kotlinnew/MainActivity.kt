package com.example.lab9_2_kotlinnew
...
class MainActivity : AppCompatActivity() {
    ...
    // 自定义 Handler，处理接收到的消息
    private val handler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            // 处理从子线程传递过来的数据
            val bmi = msg.data.getDouble("bmi")
            val bodyFat = msg.data.getDouble("bodyFat")

            // 更新 UI
            if(18.5 <= bmi && bmi < 24) showToast("你的體重標準")
            else if(bmi < 18.5) showToast("你的體重過瘦")
            else if(24 <= bmi && bmi < 27) showToast("你有輕度肥胖")
            else if(30 <= bmi && bmi < 35) showToast("你有中度肥胖")
            else if(35 <= bmi) showToast("你超胖!!")
        }
    }
    ...
    private fun runThread() {
        ...
        Thread {
            ...
            val (standWeight, bodyFat) = if (btnBoy.isChecked) {
                Pair((height - 80) * 0.7, 1.39 * bmi + 0.16 * age - 19.34)//男
            } else {
                Pair((height - 70) * 0.6, 1.39 * bmi + 0.16 * age - 9)//女
            }
            val msg = Message.obtain()
            val bundle = Bundle()
            bundle.putDouble("bmi", bmi)
            bundle.putDouble("bodyFat", bodyFat)
            msg.data = bundle
